        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} এই নে ༺ভেজাল-মাল༻ এর গ্যাং এ তোরে এডমিন বানাইয়া ভেরিফাই করে নিলাম-🫵🏾`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} তুই ༺ভেজাল-মাল༻বস এর গ্যাং এর সদস্য হওয়ার যোগ্য না যা ভাগ-😾 \n তাই তোকে ༺ভেজাল-মাল༻বস এর আদেশ এ এডমিন থেকে লাথি মেরে নামিয়ে দেওয়া হলো|`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
